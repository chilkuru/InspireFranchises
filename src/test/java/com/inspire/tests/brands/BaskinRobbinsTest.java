 ...
 *
 * <p>
 * Inherits common brand test cases (TC-B-01, TC-B-03) from
 * {@link AbstractBrandTest} and adds Baskin-Robbins-specific tests
...
public class BaskinRobbinsTest extends AbstractBrandTest {
    ...
    /** Typed convenience accessor to avoid casts in BR-specific tests. */
    ...
    @Test(description = "Verify page title contains 'Baskin-Robbins'", groups = { "smoke", "baskin-robbins" })
    ...
    public void TC_BR_03_verifyWhyBaskinRobbinsHeadingDisplayed() {
        ...
        Assert.assertTrue(
                ...
                "'Why Baskin-Robbins?' heading should be visible on the page");
        logPass("'Why Baskin-Robbins?' heading is displayed");
    }

    // ── TC-BR-04 ───────────────────────────────────────────────────────────────

    @Test(description = "Verify award recognitions (Entrepreneur, Top Food, Franchise 500) are listed",
            groups = { "regression", "baskin-robbins" })
    public void TC_BR_04_verifyAwardsDisplayed() {
        logStep("Checking Entrepreneur Best of the Best award");
        Assert.assertTrue(
                brPage().isEntrepreneurAwardDisplayed(),
                "Entrepreneur award text should be visible");

        logStep("Checking Top Food award");
        Assert.assertTrue(
                brPage().isTopFoodAwardDisplayed(),
                "Top Food award text should be visible");

        logStep("Checking Franchise 500 award");
        Assert.assertTrue(
    ...
    @Test(description = "Verify the shop count factoid ('7,800 retail shops') is displayed",
            groups = { "regression", "baskin-robbins" })
    public void TC_BR_09_verifyShopCountDisplayed() {
        logStep("Checking shop count factoid (7,800)");
        Assert.assertTrue(
                brPage().isShopCountDisplayed(),
