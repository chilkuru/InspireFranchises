...
public class BaskinRobbinsTest extends AbstractBrandTest {
    ...
    public void TC_BR_07_verifyRestaurantFormatsDisplayed() {
        ...
        Assert.assertTrue(brPage().isFreeStandingFormatDisplayed(), "'Free Standing' format should be visible");

        logStep("Checking 'Endcap' format");
        Assert.assertTrue(brPage().isEndcapFormatDisplayed(), "'Endcap' format should be visible");

        logStep("Checking 'Inline' format");
        Assert.assertTrue(brPage().isInlineFormatDisplayed(), "'Inline' format should be visible");

        logStep("Checking 'Small Format'");
        Assert.assertTrue(brPage().isSmallFormatDisplayed(), "'Small Format' label should be visible");
    ...
    @Test(description = "Verify the restaurant count factoid (7,800 shops) is displayed", groups = { "regression",
            "baskin-robbins" })
    public void TC_BR_09_verifyRestaurantCountDisplayed() {
        logStep("Checking restaurant count factoid (7,800)");
        Assert.assertTrue(
                brPage().isRestaurantCountDisplayed(),
                "'7,800' restaurant count factoid should be visible on the Baskin-Robbins page");
